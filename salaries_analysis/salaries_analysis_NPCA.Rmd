...
# drop records associated with more than one state 
...
```{r}
# function to replicate outlier conditions
find_outlier <- function(x) {
  return(x < quantile(x, .25) - 1.5*IQR(x) | x > quantile(x, .75) + 1.5*IQR(x))
}

# create column to indicate outliers
salaries2 <- salaries2 %>%
        group_by(industry) %>%
...
# Manufacturing, Financial, Tech, Government, Health care
# summary(top_10p)
```
...
needs to be normally distributed and no significant difference in variance
```{r, warning=FALSE}
# check for normal distribution visually
ggqqplot(salaries3_gender$annual_salary) # Not normally distributed

```
F-test to confirm that we can't do a t-test
```{r}
# use F-test to see if variances are equal between groups
res.ftest <- var.test(annual_salary ~ gender, data = salaries3_gender)
res.ftest # less than the significance level 0.05, therefore there is a significant difference between the two variances.
```

We can't use the t-test for this group of industries, but can we do it for the full dataset?
```{r, warning=FALSE}
# Go back to using salaries
...
# check for normal distribution visually
ggqqplot(salaries_outliers_removed$annual_salary) 
...
We can still take a look to see if there are differences in pay by industry, but we can't say that gender is the cause of any observable difference. If we had been able to do a t-test we would have wanted to further control for years of experience and education.
```{r}
...
Averages for men are higher but not significantly so. 
...
### What about pay within the nonprofit industry?
```{r, fig.height= 5}
# fig.height= 5
